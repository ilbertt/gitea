...
writable volume and no shell, package manager or `git`. The `nibrun` subcommand
carries a static git and its own hooks inside the binary, so the release asset is
